...
//for the mongodb
const mongoURI =
  "mongodb+srv://btosc:Btosc@cluster0.bnj4i.mongodb.net/ecurrency";
            ...
            passport.authenticate("local", function (err, user, info) {
              if (user) {
                ...
                let string = encodeURIComponent(
                  "UserName Or Passport is incorrect !"
                );
                req.logOut();
                if (req.params.type === "admin") {
  ...
  if (password === repassword) {
    User.register({ username: username }, password, async function (err, user) {
      if (err) {
        ...
        let string = encodeURIComponent(
          "Password change successfull!! Please login with the new password!"
        );
        res.redirect("/login/customer?valid=" + string);
      }
    });
...
app.get("/blockUser/:id", async function (req, res) {
  if (req.isAuthenticated && req.user.type === "admin") {
    ...
    var string = encodeURIComponent(
      `${user.first_name}'s Status Change Successful!`
    );
